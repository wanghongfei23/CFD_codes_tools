# 自动检测并忽略大于 100MB 的文件
# 命令行：python .inLinux_admin_script/auto_ignore_large_files.py
...
def update_gitignore(files_to_ignore, gitignore_path='.gitignore'):
    ...
    if os.path.exists(gitignore_path):
        with open(gitignore_path, 'r', encoding='utf-8') as f:
            ...
            existing_patterns = set(line.strip() for line in existing_content.split('\n') if line.strip())
    ...
    if not new_patterns:
        print(f"{Colors.GREEN}✓ 没有新的大文件需要添加到 .gitignore{Colors.END}")
        return
...
def main():
    ...
    print(f"{Colors.CYAN}{Colors.BOLD}{'自动忽略大文件工具':^100}{Colors.END}")
    print(f"{Colors.CYAN}{'=' * 100}{Colors.END}")
